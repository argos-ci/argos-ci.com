...
date: 2026-08-04
image: ./main.jpg
imageAlt: "Abstract illustration of four screenshots each showing a different flakiness artifact, followed by one clean stable frame"
---
...
**Retries shown separately.** Playwright test retries are displayed separately in the Argos UI, so a test that passed on retry is visible as such rather than hiding intermittent failures. Combined with the [Playwright trace viewer integration](/stabilize#debug), you can open the trace of the flaky run and see exactly what the page was doing at capture time.
