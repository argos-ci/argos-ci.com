...
date: 2026-05-19
image: ./main.jpg
imageAlt: "Abstract illustration of an AI agent reading CLI output and flagging one screenshot in a build"
---

Visual regression testing is no longer a purely manual task. With the right setup, AI agents can fetch Argos build data, reason about screenshots in the context of a pull request, and even submit a build review on your behalf. This guide offers a clear, step-by-step process for enabling AI-assisted reviews in your workflow. It builds on the official [AI agent review documentation](/docs/learn/review-workflow/review-builds-with-ai-agents) and uses only commands and features that currently exist in Argos, ensuring every instruction is actionable.
...
1. **A pull request with an Argos build** – either through a status check or a bot comment. Start with the [getting started guide](/docs) if Argos is not already connected to your project.
2. **The [Argos CLI](/docs/sdks-reference/argos-command-line-interface-cli)** – installed via `npm i --save-dev @argos-ci/cli` or using `npx @argos-ci/cli`.
3. **[Argos agent skills](/docs/sdks-reference/argos-command-line-interface-cli#ai-agent-skills)** – install them if your assistant supports skills.
...
Agents need credentials to read build data and (optionally) create reviews. Two types of tokens are involved; the [CLI authentication documentation](/docs/sdks-reference/argos-command-line-interface-cli#authentication) explains how Argos resolves them.
...
- **User token** – only necessary if you want the agent to submit a build review. Run the CLI login locally to store a user token:
  ...
  The `argos login` command opens a browser for OAuth and saves your token to `~/.config/argos-ci/config.json`. Do **not** run it in CI; use environment variables or [GitHub OIDC authentication](/docs/learn/integrations/github-oidc-authentication) instead.
...
Once invoked, the agent performs several actions to produce a meaningful review. The [AI agents documentation](/docs/learn/review-workflow/review-builds-with-ai-agents) outlines this sequence:
...
9. **Optionally submit the review** – if authenticated with a user token, call `argos review create <buildRef> --event <approve|reject>` to record the decision.
...
This prompt explicitly tells the assistant to follow the same steps encoded in the skills. If review submission is needed, ensure the CLI is authenticated as an Argos user or provide a personal token via `--token`; see the [project token and personal access token details](/docs/sdks-reference/argos-command-line-interface-cli#project-tokens-and-personal-access-tokens).
...
| Submit a build review    | `argos review create <buildRef> --event approve`         |
| Request changes          | `argos review create <buildRef> --event reject`          |
...
These commands are described in the [CLI documentation](/docs/sdks-reference/argos-command-line-interface-cli#build-commands). Remember that reading build data requires a project token, while submitting a review requires a personal access token.
...
Argos still performs [deterministic, pixel-perfect visual comparison](/docs/learn/platform-fundamentals/how-argos-detects-visual-differences); the agent simply uses the data to assist decision-making. If a change looks unstable, use Argos' [flaky test detection](/docs/learn/reliability-and-flakiness/flaky-test-detection) signals before approving it. Ultimately the choice to approve or request changes remains yours, but with the help of an agent you can focus on the parts that need human judgement.
