...
export const ArgosEmblem = (props: React.ComponentProps<"svg">) => {
  // Scoped per instance for the same reason as ArgosLogo: pages render several
  // emblems, and they shared their gradient ids with the logo in the navbar.
  const uid = React.useId();
  const lens = `argos-emblem-lens-${uid}`;
  const iris = `argos-emblem-iris-${uid}`;
  const ring = `argos-emblem-ring-${uid}`;
  const core = `argos-emblem-core-${uid}`;
  return (
    <svg
