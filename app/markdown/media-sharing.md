...
test run — a file with a stable share URL and Markdown ready to paste. It
...
a build. It complements [visual testing](https://argos-ci.com/visual-testing)
— builds catch the changes nobody intended, media shows the result someone did
intend.
...
- The CLI compresses PNG and JPEG to WebP client-side before upload (a 252 KB
  ...
  out. Argos never rewrites the stored file server-side — a media is usable
  the moment the upload finishes.
- A **video** embeds as its poster frame wrapped in a link to the share page —
  the only form GitHub renders — with the poster derived from the video
  itself, available immediately.
...
- Visibility governs the share page: `team` (Pro default — sign-in with
  project access required) or `public` (anyone with the URL; the only option
  on Hobby). Links carry unguessable tokens and are never indexed. The file
  bytes themselves are always reachable at an unguessable CDN URL, whatever
  the visibility — required for GitHub's server-side image proxy — so don't
  upload what must never be reachable by URL.
...
- [Upload media — CLI, SDK, API, agents](https://argos-ci.com/docs/learn/media/standalone-media-upload)
